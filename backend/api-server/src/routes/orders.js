...
import { CreateOrderBody, UpdateOrderBody, UpdateOrderParams, GetOrderParams, ListOrdersQueryParams, } from "api-zod";
const router = Router();
...
router.put("/orders/:id", authMiddleware, async (req, res) => {
    ...
    const updated = await db.orders.findOneAndUpdate({ id: params.data.id, userId: req.userId }, { $set: { ...parsed.data, updatedAt: new Date() } }, { returnDocument: "after" });
    if (!updated) {
        res.status(404).json({ error: "Order not found" });
        return;
    }

...
import { db, insertWithId, stripMongoId, type Customer } from "db";
...
const getCustomerStats = async (customerId: number, userId: number) => {
...
const withStats = async (customer: Customer, userId: number) => ({
...
router.get("/customers", authMiddleware, async (req, res): Promise<void> => {
  ...
  const filter: Filter<Customer> = { userId: req.userId! };
  if (search) filter.name = { $regex: escapeRegExp(search), $options: "i" } as never;
...
router.get("/customers/:id/history", authMiddleware, async (req, res): Promise<void> => {
  ...
  const orderProductMap = new Map<number, string[]>();
  ...
  const invoiceProductMap = new Map<number, string[]>();
...
router.put("/customers/:id", authMiddleware, async (req, res): Promise<void> => {

...
import { db, insertWithId, stripMongoId, type Invoice } from "db";
...
function formatInvoice(inv: Invoice & { itemCount?: number | string }) {
  return {
    id: inv.id,
...
router.get("/billing/invoices", authMiddleware, async (req, res): Promise<void> => {
  ...
  const filter: Filter<Invoice> = { userId: req.userId! };
  if (status) filter.status = status;
  if (search) filter.customerName = { $regex: escapeRegExp(search), $options: "i" } as never;
...
router.get("/billing/invoices/:id", authMiddleware, async (req, res): Promise<void> => {
...
router.post("/billing/invoices", authMiddleware, async (req, res): Promise<void> => {
  const { customerId, customerName, customerEmail, customerPhone, customerAddress, staffId, staffName, logoUrl, taxRate, notes, dueDate, items } = req.body;
  ...
  if (logoUrl && (typeof logoUrl !== "string" || !logoUrl.startsWith("https://res.cloudinary.com/"))) {
    res.status(400).json({ error: "Logo must be a Cloudinary image URL" });
    return;
  }
  ...
  if (!resolvedCustomerId) {
    ...
    if (!existingCustomer) {
      existingCustomer = await db.customers.findOne(
        { name: { $regex: `^${escapeRegExp(customerName.trim())}$`, $options: "i" }, userId: req.userId! },
        { projection: { id: 1 } },
      );
    }
  ...
  const mergedItems = new Map<number, { productId: number; productName?: string; sku?: string; description?: string; quantity: number; unitPrice: number }>();
  for (const item of items as Array<{ productId?: number; productName?: string; sku?: string; description?: string; quantity: number; unitPrice: number }>) {
    if (!item.productId) continue;
    const existing = mergedItems.get(item.productId);
    if (existing) existing.quantity += item.quantity;
    else mergedItems.set(item.productId, { productId: item.productId, productName: item.productName, sku: item.sku, description: item.description, quantity: item.quantity, unitPrice: item.unitPrice });
  }
...
router.patch("/billing/invoices/:id/status", authMiddleware, async (req, res): Promise<void> => {
  const id = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid id" }); return; }
  ...
  const updateValues: Partial<Invoice> = { status, updatedAt: new Date() };
  if (status === "paid") updateValues.paidAt = new Date();

  const updated = await db.invoices.findOneAndUpdate(
...
router.delete("/billing/invoices/:id", authMiddleware, async (req, res): Promise<void> => {
  const id = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid id" }); return; }
...
router.get("/billing/stats", authMiddleware, async (req, res): Promise<void> => {

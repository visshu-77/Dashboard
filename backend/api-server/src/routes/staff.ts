...
router.get("/staff", authMiddleware, async (req, res): Promise<void> => {
  const staff = await db.staff.find({ userId: req.userId! }).sort({ createdAt: 1 }).toArray();
  res.json(stripMongoIds(staff));
});

router.post("/staff", authMiddleware, async (req, res): Promise<void> => {
...
router.put("/staff/:id", authMiddleware, async (req, res): Promise<void> => {
  ...
  const updated = stripMongoId(await db.staff.findOneAndUpdate(
...
router.delete("/staff/:id", authMiddleware, async (req, res): Promise<void> => {

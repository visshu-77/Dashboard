...
import { db, insertWithId, stripMongoId, type Category, type Product } from "db";
import {
  CreateProductBody,
  UpdateProductBody,
  UpdateProductParams,
...
const buildProductFilter = (userId: number, query: { search?: string; categoryId?: number; lowStock?: boolean }): Filter<Product> => {
  const filter: Filter<Product> = { userId };
  if (query.search) filter.name = { $regex: escapeRegExp(query.search), $options: "i" } as never;
  if (query.categoryId) filter.categoryId = query.categoryId;
  if (query.lowStock) (filter as Filter<Product> & { $expr: unknown }).$expr = { $lte: ["$stock", "$minStock"] };
  return filter;
};

const categoryMapForProducts = async (userId: number, products: Product[]): Promise<Map<number, Category>> => {
  const categoryIds = [...new Set(products.map((p) => p.categoryId).filter((id): id is number => typeof id === "number"))];
...
const withCategoryNames = async (userId: number, products: Product[]) => {
...
const toUpdateProductDbValues = (product: {
  ...
  if (product.cost !== undefined) values.cost = product.cost;
  if (product.stock !== undefined) values.stock = product.stock;
  if (product.minStock !== undefined) values.minStock = product.minStock;
  if (product.categoryId !== undefined) values.categoryId = product.categoryId ?? null;
  if (product.brand !== undefined) values.brand = product.brand;
  if (product.imageUrl !== undefined) values.imageUrl = product.imageUrl;
  if (product.isActive !== undefined) values.isActive = product.isActive;
  return values;
};
...
router.get("/products/export", authMiddleware, async (req, res): Promise<void> => {
  ...
  const csv = [
    headers.join(","),
    ...items.map((product) => headers.map((header) => escapeCSVValue(product[header as keyof typeof product])).join(",")),
  ].join("\n");
...
router.post("/products", authMiddleware, async (req, res): Promise<void> => {
...
router.get("/products/download-template", (_req, res): void => {
...
router.put("/products/:id", authMiddleware, async (req, res): Promise<void> => {
  ...
  const updated = await db.products.findOneAndUpdate(
    ...
    { $set: { ...toUpdateProductDbValues(parsed.data), updatedAt: new Date() } },
...
router.post("/products/bulk-upload", authMiddleware, upload.single("file"), async (req, res): Promise<void> => {
  if (!req.file) {
  ...
  try {
    ...
    const duplicateSKUs = validRows.filter((row) => {
      if (skuSet.has(row.sku)) return true;
      skuSet.add(row.sku);
      return false;
    });
    ...
    const categoryIds = [...new Set(validRows.map((row) => row.categoryId).filter((value): value is number => value !== undefined))];
    const categoryNames = [...new Set(validRows.map((row) => row.categoryName?.trim()).filter((value): value is string => Boolean(value)))];
    const categoryFilters: Filter<Category>[] = [];
    if (categoryIds.length > 0) categoryFilters.push({ id: { $in: categoryIds } });
    if (categoryNames.length > 0) categoryFilters.push({ name: { $in: categoryNames } });
